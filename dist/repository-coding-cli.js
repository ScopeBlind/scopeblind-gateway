...
var object4 = (value) => !!value && typeof value === "object" && !Array.isArray(value);
var shape2 = (value, required, optional = []) => object4(value) && required.every((key4) => key4 in value) && Object.keys(value).every((key4) => required.includes(key4) || optional.includes(key4));
...
var at4 = (value) => typeof value === "string" && Number.isFinite(Date.parse(value)) && new Date(value).toISOString() === value;
var span2 = (issued, expires, max) => at4(issued) && at4(expires) && Date.parse(String(expires)) > Date.parse(String(issued)) && Date.parse(String(expires)) - Date.parse(String(issued)) <= max;
...
function codingTree(entries, changes) {
  ...
  const tree = (prefix) => {
    ...
    const data = Buffer.concat(rows.map((r) => Buffer.concat([Buffer.from(`${r.type === "tree" ? "40000" : r.mode} ${r.path}\0`), Buffer.from(r.sha, "hex")]))), sha2 = hash(Buffer.concat([Buffer.from(`tree ${data.length}\0`), data]));
    objects.push({ path: prefix, sha: sha2, entries: rows });
    return sha2;
  };
...
var RepositoryCodingRunner = class {
  ...
  publicationUntil = 0;
  stopped = false;
  async rpc(action, id6, body = {}) {
    const request = await sign(makeRequest(action, id6, body), this.identity), r = await this.fetcher(this.config.endpoint, { method: "POST", headers: { "content-type": "application/json", "x-scopeblind-action": action }, body: JSON.stringify({ request }), redirect: "error", signal: AbortSignal.timeout(55e3) }), v = await bounded(r);
...
`, commitSha = hash(Buffer.concat([Buffer.from(`commit ${Buffer.byteLength(commitBody)}\0`), Buffer.from(commitBody)]));
      ...
      for (const object6 of next.objects) {
        const made = await this.github("/git/trees", { method: "POST", body: JSON.stringify({ tree: object6.entries }) });
        need(made.sha === object6.sha, "coding_tree_mismatch");
      }
...
async function runRepositoryCoding(args = process.argv.slice(2)) {
  ...
  const raw = file ? await (0, import_promises.readFile)(file, "utf8") : process.env.SCOPEBLIND_CODING_CONNECTION, key4 = process.env.SCOPEBLIND_CODING_WORKER_KEY, token = process.env.GITHUB_TOKEN;
  need(raw && key4 && token, "coding_configuration_required");
  const config = JSON.parse(raw), identity = await importIdentity(key4, config.worker_key);
