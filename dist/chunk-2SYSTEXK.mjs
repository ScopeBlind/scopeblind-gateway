...
var CoordinationClient = class {
  ...
  async checkedNegotiation(response) {
    ...
    const envelope = (value) => keys(value, ["payload", "signer", "digest", "signature"]);
    const humanEnvelope = (value) => keys(value, ["payload", "signer", "digest", "signature", "authorization", "authorization_signature", "authorization_use"]);
    try {
      ...
      keys(session, ["type", "id", "room_id", "agreement_digest", "fixture_digest", "owner_key", "registrar_key", "invitation_digest", "created_at", "expires_at", "max_proposals", "parent_session_id", "source_operation_id", "source_invoice_id", "source_operation_digest"]);
      ...
      keys(invitation, ["type", "session_id", "room_id", "agreement_digest", "fixture_digest", "issuer", "registrar_key", "role", "token_hash", "max_claims", "expires_at", "parent_session_id", "source_operation_id"]);
      ...
      if (state.binding) {
        ...
        keys(binding, ["type", "session_id", "room_id", "invitation_digest", "guest_key", "name", "issued_at", "expires_at", "claim"]);
        ...
        keys(binding.claim.payload, ["type", "session_id", "room_id", "guest_key", "name", "issued_at", "nonce"]);
      }
      for (const principal of state.principals) {
        ...
        if (principal.mandate) {
          ...
          for (const required of mandate.required_invoices) {
            keys(required, ["invoice_id", "expected"]);
      ...
      for (const pair of state.viewer.pairs) {
        ...
        if (pair.readiness) {
          keys(pair.readiness, ["observed_at", "context_digest"]);
      ...
      for (const signed of agentBindings) {
        envelope(signed);
        const b = signed.payload, auth = b.owner_authorization, q = auth?.payload;
        ...
        envelope(auth);
        keys(q, ["type", "action", "room_id", "body", "issued_at", "nonce"]);
        keys(q.body, ["session_id", "pair_id", "secret_hash", "name", "expires_at", "token_expires_at", "scope", "expected_agent_key"]);
      ...
      for (let index = 0; index < state.proposals.length; index++) {
        ...
        envelope(signed);
        keys(proposal, ["type", "id", "approval_above_minor", "budget_minor", "exploration", "parent_digest", "session_id", "room_id", "round", "principal_key", "agent_key", "agent_mode", "agreement_digest", "fixture_digest", "mandate_digests", "next_agreement", "next_agreement_digest", "reviewer_grant", "reviewer_grant_digest", "issued_at"]);
      ...
      for (const signed of state.responses) {
        const response2 = signed.payload;
        envelope(signed);
        keys(response2, ["type", "session_id", "principal_key", "proposal_digest", "mandate_digest", "decision", "agent_key", "agent_mode", "issued_at"]);
        if (!actorBound(response2) || !await verify(signed, this.#config.authorityKey) || response2.type !== "scopeblind.coordination.negotiation-response.v1" || response2.session_id !== sessionId || !["support", "no_agreement"].includes(response2.decision) || !state.proposals.some((p) => p.digest === response2.proposal_digest) || state.principals.find((p) => p.key === response2.principal_key)?.mandate?.digest !== response2.mandate_digest) invalid();
      }
      for (const signed of state.reports) {
        const report = signed.payload;
        envelope(signed);
        keys(report, ["type", "session_id", "room_id", "proposal_digest", "agreement_digest", "fixture_digest", "mandate_digests", "cases_digest", "runtime_revision", "adapter", "isolation", "issued_at", "before_approval_above_minor", "after_approval_above_minor", "before_budget_minor", "after_budget_minor", "results", "required_passed", "expectations_met", "mandates_met"]);
      ...
      for (const signed of state.approvals) {
        ...
        humanEnvelope(signed);
        keys(approval, ["type", "session_id", "principal_key", "proposal_digest", "report_digest", "next_agreement_digest", "mandate_digests", "decision", "issued_at", "expires_at", "selection_basis"]);
