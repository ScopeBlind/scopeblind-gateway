...
var REPOSITORY_DEVICE_ACTIONS = ["repository_device_link_request", "repository_device_link_status", "repository_device_authorize", "repository_device_confirm", "repository_device_list", "repository_device_revoke"];
...
function validRepositoryDeviceLink(v) {
  return obj(v) && exact(v, ["type", "id", "workspace_id", "device_key", "name", "secret_hash", "issued_at", "expires_at"]) && v.type === "scopeblind.repository.device-link.v1" && id(v.id) && id(v.workspace_id) && key(v.device_key) && key(v.secret_hash) && name(v.name) && period(v, REPOSITORY_DEVICE_LINK_MS);
}
...
function validRepositoryDeviceConfirmation(v) {
  return obj(v) && exact(v, ["type", "authorization_digest", "workspace_id", "device_key", "issued_at"]) && v.type === "scopeblind.repository.device-confirmation.v1" && key(v.authorization_digest) && key(v.device_key) && id(v.workspace_id) && time(v.issued_at);
}
...
function repositoryHumanPrincipal(value) {
  return value.repository_authorization?.payload.principal_key ?? value.signer;
}
function sameRepositoryHumanIntent(left, right) {
...
var shape2 = (v, required, optional = []) => object3(v) && required.every((k) => Object.prototype.hasOwnProperty.call(v, k)) && Object.keys(v).every((k) => required.includes(k) || optional.includes(k));
var text3 = (v, max, empty = false) => typeof v === "string" && (empty || v.trim().length > 0) && v.length <= max && !/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/.test(v);
var line = (v, max) => text3(v, max) && !/[\r\n\t]/.test(String(v));
var hex2 = (v) => typeof v === "string" && REPOSITORY_HEX.test(v);
...
function content(v) {
  if (!text3(v.brief, 4e3) || !Array.isArray(v.success_criteria) || v.success_criteria.length < 1 || v.success_criteria.length > 20 || !v.success_criteria.every((c) => shape2(c, ["id", "text"]) && id3(c.id) && text3(c.text, 600)) || new Set(v.success_criteria.map((c) => c.id)).size !== v.success_criteria.length) return false;
...
function validRepositoryReviewContent(v) {
  return shape2(v, ["brief", "success_criteria"], ["preview_policy"]) && content(v);
}
function validRepositoryReviewBrief(v, task, taskDigest) {
  return shape2(v, ["type", "task_id", "task_digest", "owner_key", "brief", "success_criteria", "issued_at", "expires_at"], ["preview_policy"]) && v.type === "scopeblind.repository.review-brief.v1" && v.task_id === task.id && v.task_digest === taskDigest && v.owner_key === task.owner_key && content(v) && span2(v.issued_at, v.expires_at, 7 * 864e5) && Date.parse(String(v.issued_at)) >= Date.parse(task.issued_at) && Date.parse(String(v.expires_at)) <= Date.parse(task.expires_at);
}
...
function validRepositoryReviewFeedback(v, brief, packet) {
  return shape2(v, ["type", "id", "task_id", "task_digest", "basis_digest", "packet_digest", "requester_key", "criterion_ids", "message", "requested_changes", "issued_at"], ["mandate_digest"]) && v.type === "scopeblind.repository.review-feedback.v1" && id3(v.id) && v.task_id === brief.payload.task_id && v.task_digest === brief.payload.task_digest && hex2(v.basis_digest) && v.packet_digest === packet.digest && hex2(v.requester_key) && (v.mandate_digest === void 0 || hex2(v.mandate_digest)) && Array.isArray(v.criterion_ids) && v.criterion_ids.length <= 20 && new Set(v.criterion_ids).size === v.criterion_ids.length && v.criterion_ids.every((cid) => brief.payload.success_criteria.some((c) => c.id === cid)) && text3(v.message, 2e3) && text3(v.requested_changes, 4e3, true) && at2(v.issued_at) && Date.parse(v.issued_at) >= Date.parse(packet.payload.observed_at);
}
function validRepositoryReviewRecommendation(v, brief, packet, proposal) {
  ...
  return v.criteria.every((c) => shape2(c, ["criterion_id", "verdict", "evidence_refs", "explanation"]) && brief.payload.success_criteria.some((b) => b.id === c.criterion_id) && ["met", "not_met", "unknown"].includes(String(c.verdict)) && text3(c.explanation, 1e3) && Array.isArray(c.evidence_refs) && c.evidence_refs.length <= 12 && c.evidence_refs.every((r) => {
    if (!object3(r)) return false;
    if (r.kind === "check") return shape2(r, ["kind", "id"]) && proposal.payload.checks.some((check) => check.id === r.id);
    if (r.kind === "file") return shape2(r, ["kind", "path"]) && proposal.payload.files.some((file) => file.path === r.path);
    if (r.kind === "deployment") return shape2(r, ["kind", "id"]) && packet.payload.preview.status === "available" && packet.payload.preview.deployment.deployment_id === r.id;
    return r.kind === "artifact" && shape2(r, ["kind", "sha256"]) && packet.payload.preview.status === "available" && packet.payload.preview.artifact?.sha256 === r.sha256;
  }));
}
async function verifyRepositoryPreviewDiscovery(value, pins, now = Date.now()) {
  try {
    const e = value, v = e.payload;
    return validRepositoryEnvelope(e) && shape2(v, ["type", "repository", "pull_number", "head_sha", "owner_key", "receiver_key", "authority_key", "candidates", "checks", "status", "observed_at", "expires_at"]) && v.type === "scopeblind.repository.preview-discovery.v1" && v.repository === pins.repository && num(v.pull_number) && (pins.pull_number === void 0 || v.pull_number === pins.pull_number) && sha2(v.head_sha) && v.owner_key === pins.owner_key && v.receiver_key === pins.receiver_key && v.authority_key === pins.authority_key && span2(v.observed_at, v.expires_at, 864e5) && Date.parse(v.observed_at) <= now + 1e3 && Date.parse(v.expires_at) > now && ["observed", "unavailable"].includes(v.status) && Array.isArray(v.candidates) && v.candidates.length <= 20 && v.candidates.every((c) => shape2(c, ["environment", "origin", "environment_url", "deployment_id", "status_id"]) && line(c.environment, 100) && safeRepositoryPreviewUrl(c.environment_url) && new URL(c.environment_url).origin === c.origin && num(c.deployment_id) && num(c.status_id)) && new Set(v.candidates.map((c) => c.deployment_id)).size === v.candidates.length && Array.isArray(v.checks) && v.checks.length <= 100 && v.checks.every((c) => shape2(c, ["name", "app_id", "id"]) && line(c.name, 100) && num(c.app_id) && num(c.id)) && new Set(v.checks.map((c) => c.name + ":" + c.app_id)).size === v.checks.length && (v.status !== "unavailable" || v.candidates.length === 0) && await verify(e, pins.receiver_key);
  } catch {
    return false;
  }
}
...
async function verifyRepositoryWorkspaceAgentState(value, authorityKey, agentKey) {
  try {
    const e = value, s = e.payload;
    if (!cleanEnvelope(e) || !s || !exact3(s, ["type", "workspace", "members", "mandate", "drafts", "observed_at"]) || s.type !== "scopeblind.repository.workspace-agent-state.v1" || !await verify(e, authorityKey) || s.workspace.payload.authority_key !== authorityKey || agentKey && s.mandate.mandate.payload.agent_key !== agentKey || !await checkedMembers(s.workspace, s.members, s.observed_at) || !await checkedMandate(s.mandate, s.workspace, s.members, s.observed_at) || !Array.isArray(s.drafts) || s.drafts.length > 20 || s.drafts.length !== s.mandate.used_requests || s.drafts.filter((d) => !d.decision).length !== s.mandate.open_requests) return false;
    for (const d of s.drafts) if (!await checkedDraft(d, s.workspace, s.members, [s.mandate], s.observed_at)) return false;
    return true;
  } catch {
    return false;
  }
...
function validRepositoryCodingRefreshableConnections(v) {
  return Array.isArray(v) && v.length <= 20 && v.every((c) => object4(c) && exact4(c, ["setup_id", "worker_key"]) && id5(c.setup_id) && key3(c.worker_key)) && new Set(v.map((c) => c.setup_id)).size === v.length;
}
...
async function verifyRepositoryReviewEvidence(value, pins) {
  ...
  try {
    ...
    if (e.coding_origin) {
      ...
      check(!e.origin && shape4(o, ["job", "assignment"]) && (await verifyRepositoryCodingEvidence(o.job, key4)).published, "Coding origin is not a verified completed code job");
