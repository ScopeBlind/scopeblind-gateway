...
async function preflightCodingPullRequests(repository, read) {
  let value;
  try {
    value = await read();
  } catch {
  }
  const settings = `https://github.com/${repository}/settings/actions`;
  if (!object(value) || typeof value.can_approve_pull_request_reviews !== "boolean") throw new RepositoryReceiverError("connect_coding_pull_request_policy_unverified", `Could not verify whether GitHub Actions can create pull requests in ${repository}. Ask a repository administrator to check Settings > Actions > General > Workflow permissions at ${settings}, then rerun with a credential that can read this repository's Administration permission (or repo scope for a classic token). Coding setup cannot continue; no repository setting was changed.`);
  if (!value.can_approve_pull_request_reviews) throw new RepositoryReceiverError("connect_coding_pull_requests_disabled", `GitHub Actions cannot create pull requests in ${repository}. Ask a repository administrator to enable "Allow GitHub Actions to create and approve pull requests" under Settings > Actions > General > Workflow permissions at ${settings}. An organization administrator may need to allow this policy first. Then rerun the same setup command. Coding setup cannot continue; no repository setting was changed.`);
}
...
async function inspectRepositorySetup(request, identity, token, fetchImpl = fetch, workflowPath = REPOSITORY_GUIDED_WORKFLOW) {
  ...
  const choices = await discoverRepositoryPreviewChoices((path) => github(path, token, fetchImpl), p.repository, discovery.observed_head_sha), observed = date(), expires = new Date(Date.parse(observed) + 864e5).toISOString(), preview = await sign({ type: "scopeblind.repository.preview-discovery.v1", repository: p.repository, pull_number: p.pull_number, head_sha: discovery.observed_head_sha, owner_key: p.owner_key, receiver_key: identity.publicKey, authority_key: p.authority_key, ...choices, observed_at: observed, expires_at: expires }, identity);
...
async function installReviewedConnection(state, receiver, coding, env, execute, readyJobId) {
  ...
  if (e.coding) await preflightCodingPullRequests(repo, () => api(root + "/actions/permissions/workflow"));
...
async function runCodingReady(args, env, fetchImpl, stdout) {
  ...
  const observed = date(), ready = await sign({ type: "scopeblind.repository.setup-coding-ready.v1", setup_id: s.request.payload.id, setup_digest: s.request.digest, connection_digest: config.connection.digest, coding_digest: await sha256(canonical(coding)), worker_key: coding.worker_key, challenge_id: challenge.id, run_id: Number(env.GITHUB_RUN_ID), run_attempt: Number(env.GITHUB_RUN_ATTEMPT), workflow_ref: workflowRef, workflow_sha: env.GITHUB_WORKFLOW_SHA, observed_at: observed, expires_at: new Date(Math.min(Date.parse(observed) + 864e5, Date.parse(config.connection.payload.expires_at))).toISOString() }, identity);
