...
// Loaded lazily so `protect-mcp mcp` starts instantly and so the artifacts
// dependency is only touched when a signing/verifying tool is actually called.
let artifacts: typeof import('@veritasacta/artifacts') | null = null;
async function loadArtifacts(): Promise<typeof import('@veritasacta/artifacts')> {
...
const TOOLS = [
  {
    ...
    description:
      "Decide whether a proposed agent tool call is allowed by a Cedar policy, fail-closed. " +
      "Evaluates the call against the policy the same way the protect-mcp gate does at runtime, and on any policy error the decision is DENY (never a silent allow). " +
      "Inputs: tool (the tool name, e.g. \"Bash\" or \"send_email\"), input (the tool's arguments object; the policy sees it at context.input.*), and policy (inline Cedar source text). " +
      "Returns JSON { allowed: boolean, decision: \"allow\" | \"deny\", reason: string, policy_digest: string (sha256 prefix of the policy) }. " +
      "A missing or unparseable policy denies. Use this before an agent acts; pair with sign_decision to make the decision auditable.",
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'sign_decision',
    description:
      "Turn a gate decision into an Ed25519 signed receipt (Veritas Acta format, JCS-canonical). " +
      "A denial signs a gateway_restraint artifact; an allow signs a decision_receipt. This is what makes 'what the agent was blocked from doing' provable after the fact. " +
      "Inputs: tool (required), decision (\"allow\" | \"deny\", required), reason_code (optional), policy_digest (optional), and private_key_hex (optional 64-hex Ed25519 secret; if omitted, an ephemeral keypair is generated and its public key is returned so the receipt still verifies). " +
      "Returns JSON { receipt: object (the signed artifact), artifact_type: \"decision_receipt\" | \"gateway_restraint\", public_key: string (hex, pass this to verify_receipt or pin it), ephemeral: boolean }. " +
      "Writes nothing to disk and contacts no network.",
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'verify_receipt',
    description:
      "Verify a signed receipt offline against a public key. No network, no accounts: the Ed25519 signature is checked over the canonical bytes. " +
      "Inputs: receipt (the signed artifact object, required) and public_key_hex (optional; falls back to a public_key embedded in the receipt payload). " +
      "Returns JSON { valid: boolean, error: string | null (e.g. \"invalid_signature\", \"no_public_key\"), type: string, kid: string | null, issuer: string | null }. " +
      "For authenticity you should verify against a key you pinned out of band, not only the one carried inside the receipt.",
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'self_test',
    description:
      "Prove the gate works, end to end, with no inputs. Runs a known-forbidden action (rm -rf) against a sample deny policy and asserts it is DENIED, then signs a decision and verifies the receipt round-trips. " +
      "Returns JSON { ok: boolean, gate_denies_forbidden: boolean, sign_verify_roundtrip: boolean, details: object }. " +
      "This is the 'a gate that cannot prove it denies does not start' check, exposed as a callable tool. Contacts no network.",
    inputSchema: { type: 'object', properties: {} },
    annotations: { title: 'Prove the gate denies and receipts verify', idempotentHint: true, ...RO },
  },
];

// Mirror the receipt payload shape src/signing.ts emits, so a receipt minted
// here is byte-compatible with one the running gate signs. The spec string
// intentionally matches signing.ts (change it there, in one place, if ever).
...
async function callEvaluate(args: Record<string, unknown>): Promise<unknown> {
...
async function callSign(args: Record<string, unknown>): Promise<unknown> {
  const tool = typeof args.tool === 'string' ? args.tool : '';
  ...
  const payload = buildReceiptPayload({
    tool,
    decision,
    reason_code: typeof args.reason_code === 'string' ? args.reason_code : undefined,
    policy_digest: typeof args.policy_digest === 'string' ? args.policy_digest : undefined,
...
// The sample deny policy self_test evaluates against: a forbidden shell family.
const SELF_TEST_POLICY = `
...
async function callSelfTest(): Promise<unknown> {
  const details: Record<string, unknown> = {};

  // 1. The gate must DENY a known-forbidden action.
  const denied = await callEvaluate({ tool: 'Bash', input: { command: 'rm -rf /' }, policy: SELF_TEST_POLICY }) as { allowed?: boolean; reason?: string };
  ...
  // Sanity: a safe action under the same policy must be allowed.
  const allowed = await callEvaluate({ tool: 'Read', input: { path: './notes.txt' }, policy: SELF_TEST_POLICY }) as { allowed?: boolean };
  details.safe_action = { tool: 'Read', allowed: allowed.allowed };

  // 2. sign -> verify must round-trip; a tampered receipt must fail.
  let signVerifyRoundtrip = false;
  try {
    const signed = await callSign({ tool: 'Bash', decision: 'deny', reason_code: 'self_test' }) as { receipt?: Record<string, unknown>; public_key?: string };
...
function textResult(id: string | number | undefined, value: unknown): string {
...
async function handleRequest(request: JsonRpcRequest): Promise<string> {
  ...
  if (request.method === 'tools/call') {
    ...
    try {
      let value: unknown;
      switch (name) {
        case 'evaluate_action': value = await callEvaluate(args); break;
        case 'sign_decision': value = await callSign(args); break;
        case 'verify_receipt': value = await callVerify(args); break;
        case 'self_test': value = await callSelfTest(); break;
...
export async function runMcpServer(): Promise<void> {
  ...
  // Serialize handling so responses are written in request order.
