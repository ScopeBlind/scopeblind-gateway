/**
 * protect-mcp onboard / offboard — a guided, local-first first run.
 *
 * `onboard` walks a nontechnical operator from nothing to a governed demo and a
 * verified receipt in a few minutes, with no cloud account and no real
 * credentials: pick a policy pack, run synthetic actions through the real gate in
 * shadow mode, see one get blocked, export an evidence pack, and verify a receipt
 * offline. `offboard` is the matching one-command "delete local data / disable
 * enforcement / uninstall" path.
 *
 * Interactive on a TTY; fully non-interactive with --yes (or piped stdin) so it
 * is testable and CI-safe.
 */
import { createInterface } from 'node:readline/promises';
import { existsSync, mkdirSync, writeFileSync, appendFileSync, readFileSync, rmSync, statSync, readdirSync } from 'node:fs';
import { join } from 'node:path';
import { randomBytes } from 'node:crypto';
import { ed25519 } from '@noble/curves/ed25519';
import { bytesToHex } from '@noble/hashes/utils';
import { policySetFromSource, evaluateCedar } from './cedar-evaluator.js';
import { initSigning, signDecision } from './signing.js';
import { verifyReceipt } from './acta-envelope.js';
import { createAuditBundle } from './bundle.js';
import { POLICY_PACKS, getPolicyPack, type PolicyPack } from './policy-packs.js';

// ── colours (honour NO_COLOR) ────────────────────────────────────────────────
const c = (code: string) => (s: string) => (process.env.NO_COLOR ? s : `\x1b[${code}m${s}\x1b[0m`);
const bold = c('1'); const dim = c('2'); const green = c('32'); const red = c('31'); const yellow = c('33'); const cyan = c('36');
const out = (s = '') => process.stdout.write(s + '\n');

// ── the guided-demo scenarios (plain-English + synthetic actions) ─────────────
...
function scenarioFor(id: string): Scenario | undefined {
  return ONBOARD_PACKS.find((p) => p.id === id)?.scenario;
}

// ── small arg + prompt helpers ────────────────────────────────────────────────
...
// ── the onboard flow ──────────────────────────────────────────────────────────
export async function handleOnboard(argv: string[]): Promise<void> {
  ...
  try {
    // Step 2: pick a policy pack (the wizard)
    out(bold('  Step 2/6  Choose what the agent is allowed to do'));
    if (interactive && !packId) {
      ONBOARD_PACKS.forEach((p, i) => {
        const pack = getPolicyPack(p.id)!;
        out(`    ${cyan(String(i + 1))}) ${bold(pack.name)}  ${dim('(' + p.id + ')')}`);
    ...
    // Step 3: set up the local workspace + shadow mode
    out(bold('  Step 3/6  Set up a local workspace in ' + dir));
    const force = has(argv, '--force');
    const existingCfg = (() => {
      try { return existsSync(join(dir, 'protect-mcp.json')) ? JSON.parse(readFileSync(join(dir, 'protect-mcp.json'), 'utf-8')) : null; }
      catch { return {}; }
    })();
    // Never silently overwrite a real (non-onboarding) workspace's config and key
    // — regenerating the signing key would orphan any existing receipts.
    ...
    if (existingCfg && interactive && !yes) {
      ...
      if (go !== 'y' && go !== 'yes') { out(dim('  Cancelled. Pass --dir <path> to use a fresh folder.')); return; }
    }
    mkdirSync(join(dir, 'cedar'), { recursive: true });
    mkdirSync(join(dir, 'keys'), { recursive: true });
    const kp = keypair();
    const keyPath = join(dir, 'keys', 'gateway.json');
    writeFileSync(keyPath, JSON.stringify({ ...kp, generated_at: new Date().toISOString(), warning: 'KEEP THIS FILE SECRET.' }, null, 2) + '\n');
    ...
    writeFileSync(join(dir, 'protect-mcp.json'), JSON.stringify({
      cedar_dir: './cedar', default_tier: 'unknown',
    ...
    // Step 4: run the governed demo (real Cedar evaluation of synthetic actions)
    ...
    // Start the demo log clean, so re-running onboard (which mints a fresh key)
    // never mixes receipts from two keys and breaks the verify step.
    ...
    const rows: Array<{ label: string; tool: string; allowed: boolean; simulated: boolean }> = [];
    for (const [i, a] of scenario.demoActions.entries()) {
      const context = (a as unknown as { context?: Record<string, unknown> }).context;
      const decision = await evaluateCedar(policySet, { tool: a.tool, tier: 'unknown', toolInput: a.input, context });
      // Honest fallback: if the Cedar engine is unavailable (optional dep not
      // installed), do not label everything "blocked" (fail-closed). Use the
      // scenario's declared outcome and say it is simulated.
      let allowed = decision.allowed;
      let simulated = false;
      if (decision.metadata && (decision.metadata as { fallback?: boolean }).fallback) {
        cedarEngine = false; simulated = true; allowed = a.expect === 'allow';
      }
      // The signed receipt must self-describe honestly: a simulated decision (no
      // Cedar engine) is never labelled as a real cedar_deny in the exported pack.
      const reason_code = simulated
        ? (allowed ? 'policy_simulated_allow' : 'policy_simulated_deny')
        : (allowed ? 'policy_allow' : 'cedar_deny');
      const entry = {
        v: 2 as const, tool: a.tool, decision: allowed ? ('allow' as const) : ('deny' as const),
        reason_code, policy_digest: policySet.digest,
        request_id: `onboard-${i + 1}`, timestamp: base + i, mode: enforce ? ('enforce' as const) : ('shadow' as const),
        ...(simulated ? {} : { policy_engine: 'cedar' as const }),
      };
    ...
    // Step 5: review discovered actions
    ...
    // Step 6: export + verify + what's next
    ...
    out('  ' + (check.valid ? green('✓ receipt signature VERIFIED') : red('✗ receipt did not verify')) +
      ' with your gate\'s public key, offline (no ScopeBlind account or service).');
    out(dim('  Re-verify the whole pack yourself with the open verifier:  npx @veritasacta/verify ' + bundlePath + ' --bundle'));
    out();

    // "What ScopeBlind sees"
    ...
    out('  If you later opt into the hosted layer, it forwards the ' + bold('signed decision receipts') +
      ' themselves —');
    ...
    // Optional human approval / next steps
...
// ── the offboard flow (delete data / disable enforcement / uninstall) ─────────
export async function handleOffboard(argv: string[]): Promise<void> {
  const dir = flag(argv, '--dir') || join(process.cwd(), 'scopeblind-demo');
  const yes = has(argv, '--yes') || has(argv, '-y');
  const interactive = Boolean(process.stdin.isTTY) && !yes;
  const all = has(argv, '--uninstall') || has(argv, '--all');

  // Safety: `--uninstall` does its aggressive removal (data, keys, config,
  // policies) ONLY on a disposable onboarding workspace (marker in
  // protect-mcp.json). On a real project it removes NOTHING destructive by
  // itself — the user must ask explicitly with --delete-data / --delete-keys.
  // Removing our own hooks is always safe, so --disable-enforcement is honoured
  // regardless.
  const isOnboardingWorkspace = (() => {
    try { return JSON.parse(readFileSync(join(dir, 'protect-mcp.json'), 'utf-8'))._scopeblind_onboarding === true; }
    catch { return false; }
  })();
  ...
  // Compute exactly what will be removed, and show it first.
  const dataFiles = ['.protect-mcp-receipts.jsonl', '.protect-mcp-log.jsonl', '.protect-mcp-approval-resolutions.jsonl',
    'audit-bundle.json', 'record.html', 'verification-results.json', 'receipts'];
  const teardownFiles = ['protect-mcp.json', 'cedar', 'coverage.json'];
  const targets: string[] = [];
  if (deleteData) for (const f of dataFiles) if (existsSync(join(dir, f))) targets.push(f);
  if (deleteKeys && existsSync(join(dir, 'keys'))) targets.push('keys');
  if (fullTeardown) for (const f of teardownFiles) if (existsSync(join(dir, f))) targets.push(f);
  ...
  if (!targets.length && hookHits === 0) { out(dim('    (nothing to remove)')); out(); return; }
  out();
  ...
  if (interactive) {
    ...
    try {
      ...
      if (go !== 'y' && go !== 'yes') { out(dim('  Cancelled.')); return; }
    } finally { rl.close(); }
  }

  for (const t of targets) {
    const p = join(dir, t);
    try { rmSync(p, { recursive: statSync(p).isDirectory(), force: true }); out(green('  ✓ removed ') + t); }
    catch (e) { out(red('  ✗ could not remove ' + t + ': ' + (e instanceof Error ? e.message : e))); }
  }
  if (hookHits > 0) {
  ...
  // On a full teardown, remove the workspace folder too if it is now empty.
  if (fullTeardown && existsSync(dir)) {
    try { if (readdirSync(dir).length === 0) { rmSync(dir, { recursive: true, force: true }); out(green('  ✓ removed ') + 'the empty workspace folder'); } } catch { /* leave it */ }
  }
  out();
...
/** Count PreToolUse/PostToolUse hooks that reference protect-mcp in .claude/settings.json. */
function countProtectHooks(settingsPath: string): number {
...
function protectHookCount(hooks: unknown): number {
  ...
  for (const groups of Object.values(hooks as Record<string, unknown>)) {
    ...
    for (const g of groups) {
      ...
      if (Array.isArray(inner)) for (const h of inner) if (referencesProtect(h)) n++;
    }
  }
  return n;
}
/** True only if the hook's command/url actually invokes protect-mcp (as a token), not merely mentions the string anywhere. */
function referencesProtect(h: unknown): boolean {
  if (!h || typeof h !== 'object') return false;
  const o = h as { command?: unknown; url?: unknown };
  const cmd = typeof o.command === 'string' ? o.command : '';
  const url = typeof o.url === 'string' ? o.url : '';
  // Match the CLI token "protect-mcp" (or the "protect-mcp-mcp" server bin) only
  // when it stands alone: "npx protect-mcp evaluate", "@scopeblind/protect-mcp",
  // "/opt/protect-mcp/bin" — but NOT "protect-mcp-unrelated" or a log filename.
  const invokesProtect = /(^|[\s"'/@])protect-mcp(-mcp)?([\s"'/]|$)/.test(cmd);
  return invokesProtect || url.includes('protect-mcp');
}
/** Remove protect-mcp hook entries from .claude/settings.json; returns how many were removed. */
function removeProtectHooks(settingsPath: string): number {
  if (!existsSync(settingsPath)) return 0;
  let s: { hooks?: Record<string, unknown[]> };
  try { s = JSON.parse(readFileSync(settingsPath, 'utf-8')); } catch { return 0; }
  if (!s.hooks) return 0;
  let removed = 0;
  for (const [event, groups] of Object.entries(s.hooks)) {
    ...
    for (const g of groups) {
      ...
      if (Array.isArray(inner)) {
        const innerKept = inner.filter((h) => { if (referencesProtect(h)) { removed++; return false; } return true; });
        if (innerKept.length) kept.push({ ...(g as object), hooks: innerKept });
      } else {
        kept.push(g);
      }
    }
