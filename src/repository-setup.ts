/** Owner-local repository setup. Discovery never writes to GitHub or grants a task. */
import {canonical,COORDINATION_DOMAIN,bytesToHex,importIdentity,sha256,sign,type SigningIdentity} from './coordination-protocol.js';
import {REPOSITORY_HEX,REPOSITORY_SHA,repositoryBranch} from './coordination-repository.js';
import {validRepositoryConnection,validRepositoryReadiness,type RepositoryConnection,type RepositoryConnectionImport,type RepositoryReadiness} from './coordination-repository-collaboration.js';
import {discoverRepositoryPreviewChoices} from './repository-review-preview.js';
...
export const REPOSITORY_SETUP_VERSION='0.25.0';
...
function need(v:unknown,code:string):asserts v {if(!v)throw new RepositoryReceiverError(code);}
type RequiredCheck={name:string;app_id:number|null};
export interface RepositoryDiscovery {
 repository:string;base_branch:string;base_sha:string;observed_head_sha:string;
 checks:RepositoryReadiness['checks'];required_checks:RequiredCheck[];
 protection_read:'observed'|'unavailable';rules_read:'observed'|'unavailable';
 repository_permissions:{admin:boolean;push:boolean}|null;
 workflow_state:'present'|'absent'|'not_checked';workflow_sha?:string;workflow_sha256?:string;
 warnings:string[];
}
export interface RepositoryConnectionConfig {
 type:'scopeblind.repository.connection-config.v1';connection:RepositoryConnection;
 receiver_url:string;receiver_sha256:string;workflow_path:typeof WORKFLOW_PATH;workflow_sha256:string;
}
async function readJson(response:Response,max=2_000_000):Promise<any>{
 const reader=response.body?.getReader();need(reader,'setup_empty_response');const parts:Uint8Array[]=[];let size=0;
 try{for(;;){const next=await reader.read();if(next.done)break;size+=next.value.length;if(size>max){await reader.cancel();throw new RepositoryReceiverError('setup_response_too_large');}parts.push(next.value);}}finally{reader.releaseLock();}
 const out=new Uint8Array(size);let offset=0;for(const part of parts){out.set(part,offset);offset+=part.length;}
 try{return JSON.parse(new TextDecoder('utf-8',{fatal:true}).decode(out));}catch{throw new RepositoryReceiverError('setup_invalid_json');}
}
async function github(path:string,token:string,fetchImpl:typeof fetch):Promise<{status:number;body:any}>{
 let response:Response;try{response=await fetchImpl(`https://api.github.com${path}`,{method:'GET',headers:{Accept:'application/vnd.github+json',Authorization:`Bearer ${token}`,'X-GitHub-Api-Version':'2026-03-10'},redirect:'error',signal:AbortSignal.timeout(20000)});}catch{throw new RepositoryReceiverError('setup_github_unavailable');}
...
export async function discoverRepository(input:{repository:string;base_branch?:string;pull_number?:number;workflow_path?:'.github/workflows/scopeblind-connection.yml'|'.github/workflows/scopeblind-coding.yml'},token:string,fetchImpl:typeof fetch=fetch):Promise<RepositoryDiscovery>{
 ...
 const repo=await github(path,token,fetchImpl);need(repo.status===200&&REPO.test(repo.body.full_name)&&repo.body.full_name.toLowerCase()===input.repository.toLowerCase()&&repositoryBranch(repo.body.default_branch)&&!repo.body.archived&&!repo.body.disabled,'setup_repository_unavailable');
 const base=input.base_branch??repo.body.default_branch;need(repositoryBranch(base)&&base===repo.body.default_branch,'setup_use_trusted_default_branch');
 const branch=await github(`${path}/branches/${encodeURIComponent(base)}`,token,fetchImpl);need(branch.status===200&&branch.body.name===base&&REPOSITORY_SHA.test(branch.body.commit?.sha),'setup_base_branch_unavailable');
 let head=branch.body.commit.sha as string;
 if(input.pull_number){const pr=await github(`${path}/pulls/${input.pull_number}`,token,fetchImpl);need(pr.status===200&&pr.body.number===input.pull_number&&pr.body.state==='open'&&!pr.body.merged&&pr.body.base?.ref===base&&pr.body.base?.repo?.full_name===repo.body.full_name&&pr.body.head?.repo?.full_name===repo.body.full_name&&REPOSITORY_SHA.test(pr.body.head?.sha),'setup_same_repository_pull_required');head=pr.body.head.sha;}
 ...
 for(const run of runs.body.check_runs){need(run.head_sha===head&&safeText(run.name,100)&&Number.isSafeInteger(run.app?.id)&&run.app.id>0,'setup_invalid_check_provider');if(!checks.some(c=>c.name===run.name&&c.app_id===run.app.id))checks.push({name:run.name,app_id:run.app.id,...(safeText(run.app.name,100)?{app_name:run.app.name}:{})});}
 need(checks.length<=100,'setup_checks_incomplete');checks.sort((a,b)=>a.name.localeCompare(b.name)||a.app_id-b.app_id);
 const required:RequiredCheck[]=[];
 const add=(name:unknown,id:unknown)=>{need(safeText(name,100)&&(id===null||id===undefined||id===-1||Number.isSafeInteger(id)&&Number(id)>0),'setup_invalid_required_check');const check={name,app_id:Number.isSafeInteger(id)&&Number(id)>0?Number(id):null};if(!required.some(c=>c.name===check.name&&c.app_id===check.app_id))required.push(check);need(required.length<=100,'setup_rules_incomplete');};
...
function endpoint(value:unknown):string {let url:URL;try{url=new URL(String(value));}catch{throw new RepositoryReceiverError('setup_invalid_endpoint');}need(url.protocol==='https:'&&url.pathname==='/api/coordination'&&!url.username&&!url.password&&!url.search&&!url.hash&&url.href===value,'setup_invalid_endpoint');return url.href;}
export function parseRepositoryConnectionConfig(value:unknown):RepositoryConnectionConfig {
 const c=value as RepositoryConnectionConfig,t=c?.connection;
 ...
 need(validRepositoryConnection(t),'setup_invalid_connection_config');endpoint(t.endpoint);
 need(/^https:\/\/scopeblind\.com\/releases\/repository-receiver-[0-9]+\.[0-9]+\.[0-9]+\.cjs$/.test(c.receiver_url)&&REPOSITORY_HEX.test(c.receiver_sha256)&&c.workflow_path===WORKFLOW_PATH&&REPOSITORY_HEX.test(c.workflow_sha256),'setup_invalid_artifact_pin');return c;
}

export function renderRepositoryWorkflow(artifact:{url:string;sha256:string}):string {
...
// Hash-pinned artifact; no checkout, dependency install, or PR-controlled script.
const WORKFLOW_TEMPLATE=`# Review on the trusted default branch before enabling. Setup grants no task approval.
...
export async function createRepositoryReadiness(connection:RepositoryConnection,identity:SigningIdentity,discovery:RepositoryDiscovery,options:{now?:number;runtime?:'local'|'github_actions';workflow_sha256?:string}={}):Promise<RepositoryConnectionImport>{
 const now=options.now??Date.now();need(identity.publicKey===connection.receiver_key&&!identity.deviceAuthorization&&discovery.repository===connection.repository&&discovery.base_branch===connection.base_branch&&Date.parse(connection.expires_at)>now,'setup_connection_scope_mismatch');
 const workflow:RepositoryReadiness['workflow']=discovery.workflow_state==='absent'?'missing':discovery.workflow_state==='not_checked'?'unavailable':!options.workflow_sha256?'not_checked':discovery.workflow_sha256===options.workflow_sha256?'matching':'different';
 const readiness:RepositoryReadiness={type:'scopeblind.repository.readiness.v1',connection_digest:await sha256(COORDINATION_DOMAIN+canonical(connection)),repository:connection.repository,base_branch:connection.base_branch,owner_key:connection.owner_key,receiver_key:identity.publicKey,authority_key:connection.authority_key,checks:discovery.checks,base_sha:discovery.base_sha,check_head_sha:discovery.observed_head_sha,required_checks:discovery.required_checks,protection:discovery.protection_read,runtime:options.runtime??'local',workflow,...(discovery.workflow_sha?{workflow_sha:discovery.workflow_sha}:{}),observed_at:new Date(now).toISOString(),expires_at:new Date(Math.min(now+86400000,Date.parse(connection.expires_at))).toISOString()};
...
async function githubToken(env:NodeJS.ProcessEnv):Promise<string>{
 const value=env.GITHUB_TOKEN||env.GH_TOKEN;if(value){need(!/[\r\n\u0000]/.test(value),'setup_invalid_github_token');return value;}
 const {execFile}=await import('node:child_process');const token=await new Promise<string>((resolve,reject)=>execFile('gh',['auth','token','--hostname','github.com'],{encoding:'utf8',timeout:10000,maxBuffer:8192,env},(error,stdout)=>error?reject(new RepositoryReceiverError('setup_github_login_required','Sign in locally with gh auth login, or set GITHUB_TOKEN on this trusted machine.')):resolve(stdout.trim())));
 need(token.length>0&&!/[\r\n\u0000]/.test(token),'setup_github_login_required');return token;
}
async function localIdentity(file:string|undefined,expected:string|undefined,env:NodeJS.ProcessEnv):Promise<SigningIdentity>{
 const {readFile,stat}=await import('node:fs/promises');let privateKey=env.SCOPEBLIND_RECEIVER_PRIVATE_KEY,publicKey=expected;
 if(file){const meta=await stat(file);need(meta.isFile()&&(process.platform==='win32'||(meta.mode&0o077)===0),'setup_private_key_permissions');let key:any;try{key=JSON.parse(await readFile(file,'utf8'));}catch{throw new RepositoryReceiverError('setup_invalid_private_key_file');}need(key.type==='scopeblind.repository.receiver-key.v1'&&(!expected||key.public_key===expected),'receiver_key_mismatch');privateKey=key.private_key;publicKey=key.public_key;}
 need(typeof privateKey==='string'&&/^[0-9a-f]{96,300}$/.test(privateKey)&&typeof publicKey==='string'&&REPOSITORY_HEX.test(publicKey),'receiver_private_key_required');try{return await importIdentity(privateKey,publicKey);}catch{throw new RepositoryReceiverError('receiver_key_mismatch');}
}
async function checkServicePin(url:string,pin:string,fetchImpl:typeof fetch){let response:Response;try{response=await fetchImpl(`${endpoint(url)}?op=info`,{method:'GET',redirect:'error',signal:AbortSignal.timeout(20000)});}catch{throw new RepositoryReceiverError('setup_service_unavailable');}const info=await readJson(response,10000);need(response.ok&&info.ok===true&&info.authority_key===pin&&info.protocol==='scopeblind.coordination.v1','setup_service_pin_mismatch');}
async function artifactPin(url:string,pin:string|undefined,fetchImpl:typeof fetch):Promise<string>{
 need(/^https:\/\/scopeblind\.com\/releases\/repository-receiver-[0-9]+\.[0-9]+\.[0-9]+\.cjs$/.test(url),'setup_invalid_artifact_pin');
 if(pin){need(REPOSITORY_HEX.test(pin),'setup_invalid_artifact_pin');return pin;}
 let response:Response;try{response=await fetchImpl(`${url}.sha256`,{method:'GET',redirect:'error',signal:AbortSignal.timeout(20000)});}catch{throw new RepositoryReceiverError('setup_release_checksum_unavailable');}
 need(response.ok,'setup_release_checksum_unavailable');const value=await response.text();need(value.length<1000&&/^[0-9a-f]{64}(?:\s+[^\r\n]+)?\s*$/.test(value),'setup_invalid_release_checksum');return value.slice(0,64);
}
...
export async function runRepositoryCommand(args:string[],dependencies:{fetchImpl?:typeof fetch;env?:NodeJS.ProcessEnv;stdout?:(text:string)=>void}={}):Promise<void>{
 ...
 for(let i=1;i<args.length;i+=2){need(allowed.includes(args[i])&&!options.has(args[i])&&typeof args[i+1]==='string'&&!args[i+1].startsWith('--'),'setup_invalid_arguments');options.set(args[i],args[i+1]);}
 const env=dependencies.env??process.env,fetchImpl=dependencies.fetchImpl??fetch,stdout=dependencies.stdout??((s:string)=>process.stdout.write(s));
 const pull=options.get('--pull');if(pull!==undefined)need(/^[1-9][0-9]*$/.test(pull)&&Number.isSafeInteger(Number(pull)),'setup_invalid_pull_number');
 const token=await githubToken(env);
 if(args[0]==='ready'){
  need(options.get('--connection')&&options.get('--output'),'setup_connection_and_output_required');
  let value:unknown;try{value=JSON.parse(await readFile(options.get('--connection')!,'utf8'));}catch{throw new RepositoryReceiverError('setup_invalid_connection_config');}
  const config=parseRepositoryConnectionConfig(value),identity=await localIdentity(options.get('--key-file'),config.connection.receiver_key,env);
  await checkServicePin(config.connection.endpoint,config.connection.authority_key,fetchImpl);
  const discovery=await discoverRepository({repository:config.connection.repository,base_branch:config.connection.base_branch,...(pull?{pull_number:Number(pull)}:{})},token,fetchImpl);
  const actions=env.GITHUB_ACTIONS==='true';if(actions)need(env.GITHUB_REPOSITORY===config.connection.repository&&env.GITHUB_EVENT_NAME==='workflow_dispatch'&&env.GITHUB_REF===`refs/heads/${config.connection.base_branch}`&&discovery.workflow_sha256===config.workflow_sha256&&env.GITHUB_WORKFLOW_REF===`${config.connection.repository}/${WORKFLOW_PATH}@refs/heads/${config.connection.base_branch}`,'receiver_trusted_workflow_required');
  const result=await createRepositoryReadiness(config.connection,identity,discovery,{runtime:actions?'github_actions':'local',workflow_sha256:config.workflow_sha256});await writeFile(options.get('--output')!,JSON.stringify(result,null,2)+'\n',{mode:0o600,flag:'wx'});
  stdout(`Read-only readiness recorded for ${config.connection.repository}. No task was approved or executed. Import the public observation into the repository connection.\n`);return;
 }
 ...
 const service=endpoint(options.get('--endpoint')??'https://scopeblind.com/api/coordination');await checkServicePin(service,authority,fetchImpl);
 ...
 const directory=resolve(output);let exists=false;try{await stat(directory);exists=true;}catch(error){if((error as NodeJS.ErrnoException).code!=='ENOENT')throw error;}need(!exists,'setup_output_exists');
 await mkdir(directory,{mode:0o700});
 let keyFile=options.get('--key-file');
 if(!keyFile){keyFile=join(directory,'receiver-key.json');const pair=await crypto.subtle.generateKey({name:'Ed25519'},true,['sign','verify']) as CryptoKeyPair;const key={type:'scopeblind.repository.receiver-key.v1',public_key:bytesToHex(new Uint8Array(await crypto.subtle.exportKey('raw',pair.publicKey))),private_key:bytesToHex(new Uint8Array(await crypto.subtle.exportKey('pkcs8',pair.privateKey)))};await writeFile(keyFile,JSON.stringify(key,null,2)+'\n',{mode:0o600,flag:'wx'});}
 const identity=await localIdentity(keyFile,undefined,env);need(identity.publicKey!==owner&&identity.publicKey!==authority,'setup_independent_receiver_key_required');
 const now=Date.now(),connection:RepositoryConnection={type:'scopeblind.repository.connection.v1',id:crypto.randomUUID(),endpoint:service,repository:discovery.repository,base_branch:discovery.base_branch,owner_key:owner,receiver_key:identity.publicKey,authority_key:authority,issued_at:new Date(now).toISOString(),expires_at:new Date(now+30*86400000).toISOString()};
 const config:RepositoryConnectionConfig={type:'scopeblind.repository.connection-config.v1',connection,receiver_url:url,receiver_sha256:hash,workflow_path:WORKFLOW_PATH,workflow_sha256:await sha256(workflow)};parseRepositoryConnectionConfig(config);
 ...
 if(pull){const choices=await discoverRepositoryPreviewChoices(path=>github(path,token,fetchImpl),discovery.repository,discovery.observed_head_sha),observed=Date.now(),preview=await sign<RepositoryPreviewDiscovery>({type:'scopeblind.repository.preview-discovery.v1',repository:discovery.repository,pull_number:Number(pull),head_sha:discovery.observed_head_sha,owner_key:owner,receiver_key:identity.publicKey,authority_key:authority,...choices,observed_at:new Date(observed).toISOString(),expires_at:new Date(observed+86400000).toISOString()},identity);await writeFile(join(directory,'preview-discovery.json'),JSON.stringify(preview,null,2)+'\n',{mode:0o600,flag:'wx'});previewFile=true;}
...
function installationReview(config:RepositoryConnectionConfig,d:RepositoryDiscovery,independentPin:boolean):string {
