...
const contactPageSchema={type:'object',additionalProperties:false,properties:{type:{type:'string',const:'scopeblind.contact-page.v1'},button_label:{type:'string',minLength:1,maxLength:40},target:{type:'string',enum:['broken','contact']},accent:{type:'string',enum:['indigo','emerald','rose']}},required:['type','button_label','target','accent']};
...
export async function handleAgentProfileRequest(client:CoordinationAgentClient,request:Request,signal?:AbortSignal):Promise<unknown>{
  ...
  if(request.method==='initialize')return {jsonrpc:'2.0',id:request.id,result:{protocolVersion:'2024-11-05',serverInfo:{name:'protect-mcp-agent',version:process.env.PROTECT_MCP_VERSION||'0.24.0'},capabilities:{tools:{}}}};
  ...
  try{
    ...
    if(name==='coordination.request_repository_revision'){only('connection_id','request_id','basis_digest','message','proposed');return result(request,await client.requestRepositoryRevision(fields as unknown as Parameters<CoordinationAgentClient['requestRepositoryRevision']>[0]));}
