...
import {verifyRepositoryEvidence,type RepositoryEvidence,type RepositoryState} from './coordination-repository.js';
import {contactPageBytes,validRepositoryParticipants,validRepositoryPreview,validRepositoryAgentGrant,validRepositoryRevisionRequest,validRepositoryRevisionLink,validRepositoryDemoRequest,validRepositoryDemoProvision,repositoryRevisionBasis,CONTACT_PATH,type RepositoryCollaboration,type RepositoryDemoState,type RepositoryRevisionRequest,type RepositoryAgentGrant} from './coordination-repository-collaboration.js';

export interface RepositoryCollaborationEvidence {
 type:'scopeblind.repository.collaboration-evidence.v1';repository:RepositoryEvidence;collaboration:Signed<RepositoryCollaboration>;
 demo?:Signed<RepositoryDemoState>;parent?:RepositoryEvidence;
 parent_request?:Signed<RepositoryRevisionRequest>;parent_agent_grant?:Signed<RepositoryAgentGrant>;
}
const shape=(v:unknown,required:string[],optional:string[]=[]):v is Record<string,any>=>!!v&&typeof v==='object'&&!Array.isArray(v)&&required.every(k=>Object.hasOwn(v,k))&&Object.keys(v).every(k=>required.includes(k)||optional.includes(k));
const envelope=(v:unknown)=>shape(v,['payload','signer','digest','signature']);
const time=(v:unknown):v is string=>typeof v==='string'&&Number.isFinite(Date.parse(v))&&new Date(v).toISOString()===v;
const within=(at:string,start:string,end:string)=>Date.parse(at)>=Date.parse(start)&&Date.parse(at)<=Date.parse(end);
...
export async function verifyRepositoryCollaborationEvidence(input:unknown,pins?:Parameters<typeof verifyRepositoryEvidence>[1]){
 const result={valid:false,accepted:false,authorityPinned:false,previewVerified:false,revisionLinked:false,errors:[] as string[],limitations:[...baseLimitations]};
 try {
  ...
  const bundle=input as unknown as RepositoryCollaborationEvidence;
  const core=await verifyRepositoryEvidence(bundle.repository,pins);result.limitations.push(...core.limitations);requireValid(core.valid,core.errors.join('; '));
  ...
  const grants=new Map<string,RepositoryCollaboration['agent_grants'][number]>(),requestIds=new Set<string>(),requests=new Map<string,RepositoryCollaboration['requests'][number]>();
  ...
  for(const signed of c.requests){
   const r=signed?.payload;requireValid(envelope(signed)&&validRepositoryRevisionRequest(r)&&await verify(signed,r.requester_key)&&r.task_id===task.id&&r.task_digest===state.task.digest&&within(r.issued_at,task.issued_at,task.expires_at)&&Date.parse(r.issued_at)<=Date.parse(c.observed_at)&&!requestIds.has(r.id)&&!requests.has(signed.digest),'A revision request is invalid or duplicated.');
   requestIds.add(r.id);requests.set(signed.digest,signed);
   if(principals.includes(r.requester_key))requireValid(r.grant_digest===undefined,'A human revision must not borrow an agent grant.');
   else {const g=r.grant_digest?grants.get(r.grant_digest)?.grant.payload:undefined;requireValid(g&&g.agent_key===r.requester_key&&g.permissions.includes('request_revision')&&within(r.issued_at,g.issued_at,g.expires_at),'The suggesting agent did not hold the exact recorded revision scope.');}
  }
  let parent:RepositoryState|undefined;
  if(bundle.parent){const checked=await verifyRepositoryEvidence(bundle.parent,pins);requireValid(checked.valid,'The predecessor repository evidence does not verify.');parent=bundle.parent.state.payload;requireValid(parent.task.payload.owner_key===task.owner_key&&parent.task.payload.receiver_key===task.receiver_key&&parent.task.payload.authority_key===task.authority_key&&parent.task.payload.repository===task.repository,'The predecessor crosses this repository or participant authority.');}
  const linkIds=new Set<string>();
  for(const signed of c.revisions){
   const l=signed?.payload;requireValid(envelope(signed)&&validRepositoryRevisionLink(l)&&await verify(signed,task.owner_key)&&l.owner_key===task.owner_key&&time(l.issued_at)&&Date.parse(l.issued_at)<=Date.parse(c.observed_at)&&!linkIds.has(l.id),'A revision link is invalid or duplicated.');linkIds.add(l.id);
   if(l.child_task_id===task.id){
    ...
    if(parentPrincipals.includes(r.requester_key))requireValid(r.grant_digest===undefined&&bundle.parent_agent_grant===undefined,'Human feedback must not borrow an agent grant.');
    else {
  ...
  result.valid=true;result.accepted=core.accepted;
 }catch(error){result.errors.push(error instanceof Error?error.message:'The collaboration evidence could not be verified.');result.previewVerified=false;result.revisionLinked=false;result.accepted=false;}
 return result;
}
