...
export async function createReceiptRegistry(opts: AnchorOptions): Promise<{ registry: ReceiptRegistry; registryPath: string; verifierPath: string; uploaded: boolean }> {
  ...
  const registry: ReceiptRegistry = {
    ...
    privacy: {
      statement: uploaded
        ? 'ScopeBlind managed witness received receipt digests and explicitly listed public metadata only. This is not an independent timestamp.'
        : 'Local preview registry only. It provides no independent timing or inclusion evidence.',
