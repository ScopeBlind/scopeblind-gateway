...
import { CoordinationClient, CoordinationError, type CoordinationPayment, type CoordinationWait } from './coordination-client.js';
import type { NegotiationProposalInput } from './coordination-negotiation.js';
import type { RehearsalCase, RepairProposalInput } from './coordination-rehearsal.js';
import { coordinationConfigFromFile } from './coordination-pair-cli.js';
import { coordinationConfigFromArgs } from './coordination-config.js';
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        amount_minor: { type: 'integer', minimum: 1, maximum: 10000000, description: 'USD cents; 32000 means $320.00.' },
...
export const REHEARSAL_TOOLS = [
  ...
  },
  {
    ...
    inputSchema: { type: 'object', properties: { case: { type: 'object', properties: {
      id: { type: 'string', pattern: '^[A-Za-z0-9_-]{1,80}$' }, title: { type: 'string', minLength: 1, maxLength: 120 },
      ...
      invoice_id: { type: 'string', pattern: '^[A-Za-z0-9_-]{1,60}$' }, amount_minor: { type: 'integer', minimum: 1, maximum: 10000000, description: 'Optional explicit invoice AND matching purchase-order override inside this case’s isolated ledger only.' },
      expected: { type: 'string', enum: ['allow','ask','refuse','invariant'] }, requirement: { type: 'string', minLength: 1, maxLength: 400 },
  ...
  },
  {
    ...
    inputSchema: { type: 'object', properties: { proposal: { type: 'object', properties: { id: { type: 'string', pattern: '^[A-Za-z0-9_-]{1,80}$' }, approval_above_minor: { type: 'integer', minimum: 0, maximum: 10000000 }, rationale: { type: 'string', minLength: 1, maxLength: 600 } }, required: ['id','approval_above_minor','rationale'], additionalProperties: false } }, required: ['proposal'], additionalProperties: false },
...
/** These tools never expose payment, human approval, adoption, or hosted-model execution. */
export const NEGOTIATION_TOOLS = [
  ...
  },
  {
    ...
    inputSchema: { type: 'object', properties: { proposal: { type: 'object', properties: { id: { type: 'string', pattern: '^[A-Za-z0-9_-]{1,80}$' }, approval_above_minor: { type: 'integer', minimum: 0, maximum: 10000000, description: 'USD cents: 40000 means review above $400.' }, budget_minor: { type: 'integer', minimum: 1, maximum: 10000000, description: 'Optional total budget in USD cents, within the principal’s signed budget limits. Omit to retain the source budget.' }, parent_digest: { type: 'string', pattern: '^[0-9a-f]{64}$' } }, required: ['id','approval_above_minor'], additionalProperties: false } }, required: ['proposal'], additionalProperties: false },
  ...
  },
  {
    ...
    inputSchema: { type: 'object', properties: { after_digest: { type: 'string', pattern: '^[0-9a-f]{64}$' }, timeout_ms: { type: 'integer', minimum: 1000, maximum: 30000, default: 30000 } }, additionalProperties: false },
...
const toolsForPurpose = (purpose: CoordinationClient['purpose']) => purpose === 'negotiation' ? NEGOTIATION_TOOLS : purpose === 'rehearsal' ? REHEARSAL_TOOLS : COORDINATION_TOOLS;
...
const textResult = (id: Request['id'], value: unknown, isError = false) => ({ jsonrpc: '2.0', id, result: { content: [{ type: 'text', text: JSON.stringify(value) }], ...(isError ? { isError: true } : {}) } });
...
export async function handleCoordinationRequest(client: CoordinationClient, request: Request, signal?: AbortSignal): Promise<unknown> {
  ...
  try {
    ...
    if (request.params?.name === 'coordination.inspect_negotiation') {
      ...
      return textResult(request.id, await client.inspectNegotiation(fields.report_digest as string | undefined, signal));
    }
    if (request.params?.name === 'coordination.propose_candidate') {
      ...
      return textResult(request.id, await client.proposeCandidate(fields.proposal as NegotiationProposalInput));
    }
    if (request.params?.name === 'coordination.respond_candidate') return textResult(request.id, await client.respondCandidate(fields as { proposal_digest: string; decision: 'support' | 'no_agreement' }));
    if (request.params?.name === 'coordination.compare_candidate') return textResult(request.id, await client.compareCandidate(fields as { proposal_digest: string }, signal));
    if (request.params?.name === 'coordination.wait_negotiation') return textResult(request.id, await client.waitNegotiation(fields as { after_digest?: string; timeout_ms?: number }, signal));
    if (request.params?.name === 'coordination.inspect_rehearsal') {
      ...
      return textResult(request.id, await client.inspectRehearsal(fields.report_digest as string | undefined));
    }
    if (request.params?.name === 'coordination.propose_case') {
      ...
      return textResult(request.id, await client.proposeCase(fields.case as RehearsalCase));
    }
    if (request.params?.name === 'coordination.propose_repair') {
      ...
      return textResult(request.id, await client.proposeRepair(fields.proposal as RepairProposalInput));
    }
    if (request.params?.name === 'coordination.run_rehearsal') return textResult(request.id, await client.runRehearsal(fields as { id: string; proposal_id?: string }, signal));
    if (request.params?.name === 'coordination.wait') return textResult(request.id, await client.wait(args as CoordinationWait, signal));
    if (request.params?.name === 'coordination.deliver') {
      ...
      return textResult(request.id, await client.deliver((args as Record<string, unknown>).run_id as string));
    }
    ...
    const allowed = new Set(['operation_id', 'invoice_id', 'amount_minor', 'currency', 'destination', 'fixture_revision']);
    ...
    const payment: CoordinationPayment = {
      operation_id: values.operation_id as string,
      input: { invoice_id: values.invoice_id as string, amount_minor: values.amount_minor as number, currency: values.currency as 'USD', destination: values.destination as string, ...(values.fixture_revision !== undefined ? { fixture_revision: values.fixture_revision as number } : {}) },
    };
...
export async function runCoordinationServer(args: string[]): Promise<void> {
  ...
  const waits = new Map<Request['id'], AbortController>();
  const cancelWaits = () => { for (const controller of waits.values()) controller.abort(); };
  ...
  lines.on('line', line => {
    ...
    let request: Request;
    try {
      if (line.length > 1_000_000) throw new Error('Frame too large');
    ...
    const controller = request?.method === 'tools/call' && ['coordination.wait','coordination.run_rehearsal','coordination.wait_negotiation','coordination.compare_candidate'].includes(String(request.params?.name)) ? new AbortController() : undefined;
