...
describe('principal-scoped installed negotiation', () => {
  ...
  it('reports the negotiation-capable 0.22.0 release on MCP initialization', async () => {
    const f = await fixture(); vi.stubEnv('PROTECT_MCP_VERSION', '');
    try { const response: any = await handleCoordinationRequest(f.client, { jsonrpc: '2.0', id: 1, method: 'initialize' }); expect(response.result.serverInfo.version).toBe('0.22.0'); } finally { vi.unstubAllEnvs(); }
  });
